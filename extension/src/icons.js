// SVG 캐릭터 정의 (Mission Control - 기하학적/추상적 스타일)
...
// SVG 캐릭터 가져오기 (정규화된 타입 이름)
export function getAgentCharacter(agentType) {
  const normalized = agentType.toLowerCase()
    ...
    .replace(/^scientist-.*/, 'scientist')
    .replace(/^security-reviewer.*/, 'security-reviewer')
    .replace(/^qa-tester.*/, 'qa-tester')
    .replace(/^build-fixer.*/, 'build-fixer')
    .replace(/^tdd-guide.*/, 'tdd-guide')
    .replace(/^git-master$/, 'git-master')
...
export function getAgentDescription(agentType) {
  const normalized = agentType.toLowerCase()
    ...
    .replace(/^researcher-.*/, 'researcher')
    .replace(/^scientist-.*/, 'scientist')
    .replace(/^security-reviewer.*/, 'code-reviewer')
    .replace(/^qa-tester.*/, 'code-reviewer')
    .replace(/^build-fixer.*/, 'executor')
    .replace(/^tdd-guide.*/, 'code-reviewer')
    .replace(/^git-master$/, 'executor')
    .replace(/scout/, 'explore')
